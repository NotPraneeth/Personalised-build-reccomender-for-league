...
const runFullCollection = async () => {
    try {
        ...
        for (const { tier, division } of TARGET_TIERS) {
            ...
            const playersResponse = await axios.get(playersApiUrl);
            ...
            for (const player of players) {
                try {
                    ...
                    console.warn(`- Skipping one player's history in ${tier} ${division}.`);
                    await sleep(1500);
                }
            }
            ...
            for (const matchId of uniqueMatchIds) {
                ...
                try {
                    ...
                    const detailsResponse = await axios.get(matchDetailsUrl);
                    await sleep(1500);
                    const timelineResponse = await axios.get(matchTimelineUrl);
